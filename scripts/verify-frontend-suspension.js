...
function fixture(){
 const posts=[],writes=[],timers=new Map(),reads=[];let receive,id=0;
 const bridge={onMessage(f){receive=f;},postMessage(p){posts.push(p);}};
 const w=()=>bridge;w.onReady=()=>{};
 const c={console:{log(){},error(){},warn(){}},local:{getItem(){return null;},setItem(...a){writes.push(a);}},wixLocationFrontend:{url:'https://www.wanderlustcaribbean.com/?gclid=fixture',query:{gclid:'fixture'}},setTimeout(f){timers.set(++id,f);return id;},clearTimeout(i){timers.delete(i);},getAdvertisingSuspension(key){assert.equal(key,'suspendGoogleAds');return new Promise((resolve,reject)=>reads.push({resolve,reject}));}};
 vm.createContext(c);vm.runInContext(read('velo/public/tracking.js').replace(/^import .*;\n/gm,'').replace(/export /g,''),c);
 return {c,w,bridge,posts,writes,timers,reads,ready(){receive({data:{type:'wbe-microsoft-ready',version:1}});},tick(){const a=[...timers.values()];timers.clear();a.forEach(f=>f());}};
}
...
test('FS02',async()=>{for(const value of [null,undefined,false,true,'',{},[],2,'bad',1,'1',0,'0']){const h=fixture();h.c.initTracking(h.w);assert.equal(typeof h.c.observeTrackingSuspension,'function','shared scalar observation required');const p=h.c.observeTrackingSuspension();await Promise.resolve();h.reads[0].resolve(value);await p;h.c.trackPurchase({transactionId:'scalar',value:0,currency:'EUR'});assert.equal(h.posts.filter(p=>p.type==='wbe-datalayer-event').length,value===0||value==='0'?1:0);}});
...
test('FS05',()=>{const h=fixture();h.c.initTracking(h.w);h.c.setSuspendGoogleAds(false);h.c.trackPurchase({transactionId:'clear',value:1});h.c.setSuspendGoogleAds(true);assert.equal(h.timers.size,0);h.c.setSuspendGoogleAds(false);h.ready();assert.equal(h.posts.filter(p=>p.kind==='event').length,0);});
...
test('FS10',()=>{const h=require('./verify-event-bridge.js').setup();h.observe('/');h.connect();h.producer.trackPurchase({transactionId:'already-relayed',value:0,currency:'EUR'});h.producer.setSuspendGoogleAds(true);assert.equal(h.calls.filter(p=>p[1]==='purchase').length,0);h.complete();assert.equal(h.calls.filter(p=>p[1]==='purchase').length,1,'BLOCKER: unchanged head drains already-relayed work after producer ON');console.log('BLOCKER | FS10 | producer-only suspension cannot clear unchanged head pendingEvents; P2 incomplete');});
...
function callers(){
 ...
 for(const name of ['masterPage','page-booking-search','page-booking-summary']){
  ...
  const w=id=>{if(id==='#wbeEventBridge')return h.bridge;if(!elements.has(id))elements.set(id,{value:null,text:'',link:'',data:[],hide(){},show(){},collapse(){},expand(){},enable(){},disable(){},onClick(f){this.click=f;},onChange(f){this.change=f;},onItemReady(f){this.itemReady=f;},forEachItem(){}});return elements.get(id);};w.onReady=f=>callbacks.push(f);
 ...
 return {...h,pages,flights,ui,get listeners(){return listeners;},async start(){for(const p of Object.values(pages))for(const f of p.callbacks)await f();await settle();}};
}
...
test('FS02-A-actual-scalar-matrix',async()=>{
 for(const value of [undefined,null,true,false,'','bad',{},[],2,1,' 1 ',0,' 0 ']){
  const h=callers();await h.start();h.ready();h.reads[0].resolve(value);await settle();
 ...
 const h=callers();await h.start();h.ready();h.reads[0].reject(Error('inert scalar'));await settle();assert.equal(h.posts.length,0);assert.equal(h.writes.length,0);
});
...
test('FS05-B-reentrant-probe',()=>{for(const throws of [false,true]){const h=fixture();h.c.initTracking(h.w);h.c.trackPurchase({transactionId:'old',value:1});h.bridge.postMessage=p=>{h.posts.push(p);if(p.type==='wbe-microsoft-probe'){h.c.setSuspendGoogleAds(true);if(throws)throw Error('inert');}};h.c.setSuspendGoogleAds(false);assert.equal(h.timers.size,0);assert.equal(h.posts.length,1);assert.equal(vm.runInContext('microsoftQueue.length+googleQueue.length',h.c),0);}});
test('FS06-A-B-actual-negative-paths',async()=>{
 for(const mode of ['saved','click','server'])for(const transport of ['throw','missing']){
  ...
  const flight=h.c.observeTrackingSuspension();await settle();
  ...
  m.callbacks[0]();if(mode==='click')await m.elements.get('#btnWithdrawAdvertising').click();await settle();
  ...
  assert.equal(h.timers.size,0);assert.equal(vm.runInContext('microsoftQueue.length+googleQueue.length',h.c),0);
...
test('FS03-A-actual-handlers',async()=>{
 for(const branch of ['available','empty','unavailable']){
  ...
  await s.w('#btnSearchRooms').click();await settle();assert.equal(h.posts.length,0);
  h.reads[0].resolve(0);await settle();
...
test('FS09-A-date-UI-pricing-independence',async()=>{
 ...
 await h.start();h.reads[0].reject(Error('failed tracking observation'));await settle();
