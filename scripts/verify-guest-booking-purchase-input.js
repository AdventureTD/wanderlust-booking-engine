...
function isolatedPurchaseConsumer(file, text) {
  assert.ok(financialReaderAllowed(file, text), 'pinned private reader only: ' + file);
...
for(const f of walk(path.join(root,'velo')).filter(f=>/\.(?:js|jsw)$/.test(f)&&f!==file)) {isolatedPurchaseConsumer(path.relative(root,f).split(path.sep).join('/'),fs.readFileSync(f,'utf8'));count++;}
