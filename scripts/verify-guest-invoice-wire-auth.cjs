...
function load(rel){
 ...
 s=s.replace(/^import \{ ([^}]+) \} from '([^']+)';$/gm,(_,names,dep)=>{
  const allow=rel.endsWith('TransportAuth.js')?['wix-secrets-backend.v2','wix-auth','backend/guestBookingInvoiceAuthorityConfig','crypto','buffer']:['wix-secrets-backend.v2','wix-auth','buffer'];assert(allow.includes(dep));
...
(async()=>{
 ...
 console.log(JSON.stringify({cases,count:cases.length,loaded,scope,bodyHex:body.toString('hex'),requestMac:expectedMac,richHex:richResponse.body.toString('hex'),richMac:richResponse.headers['X-WBE-GI-Mac'],bridgeResponse}));
