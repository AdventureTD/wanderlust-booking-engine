...
BASE = Path(os.path.abspath(os.environ["WP_ROOT"]))
MODULE = BASE / 'booking_engine/guest_invoice_wire_auth.py'



def preflight():
    # Before Python file-spec evaluation as well as every Node VM evaluation.
    keys = ('WP_INSPECTOR', 'WP_INSPECTOR_SHA256', 'WP_ORIGINAL_MANIFEST', 'WP_SUPPORT_RECORD', 'WP_SUPPORT_SHA256', 'WP_SHAPE', 'WP_PYTHON')
    env = {key: os.environ[key] for key in keys}
    if hashlib.sha256(Path(env['WP_INSPECTOR']).read_bytes()).hexdigest() != env['WP_INSPECTOR_SHA256']:
        raise AssertionError('external helper anchor')
    command = [env['WP_PYTHON'], '-B', env['WP_INSPECTOR'], str(BASE), env['WP_ORIGINAL_MANIFEST'], env['WP_SUPPORT_RECORD'], env['WP_SUPPORT_SHA256'], env['WP_SHAPE']]
    run = subprocess.run(command, capture_output=True, text=True, timeout=30)
    if run.returncode:
        raise AssertionError(run.stderr)
    if json.loads(run.stdout)['classification'] != 'BYTE_INSPECTION_ONLY_NOT_BEHAVIOR':
        raise AssertionError('preflight classification')
    return json.loads(run.stdout)

if __name__ == '__main__' and '--inspect-only' in sys.argv:
    print(json.dumps(preflight(), sort_keys=True))
    sys.exit(0)


def native_node(bridge=None):
    command = ['node', str(BASE / 'scripts/private-invoice-wire-wp/verify-auth.cjs')]
