...
(async()=>{
 ...
 assert.notEqual(result,'DENIED');assert.equal(result.capsule,r.capsule,'actual issuer must reproduce exact original capsule with NEW explicit test entropy');assert.deepEqual(result.display,o.calculation);assert.equal(rngIndex,2);
