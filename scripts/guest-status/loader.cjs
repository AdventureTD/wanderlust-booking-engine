...
function fixture(name){assert.ok(Object.hasOwn(admission.fixtures,name));const b=fs.readFileSync(path.join(root,'scripts/fixtures',name));assert.equal(hash(b.toString().replace(/\r\n/g,'\n')),admission.fixtureCanonical[name]);return JSON.parse(b);}
function preflight(){
 for(const [id,pin] of Object.entries(admission.supportCanonical))assert.equal(hash(fs.readFileSync(path.join(root,id),'utf8').replace(/\r\n/g,'\n')),pin,id);
 ...
 for(const [id,pin] of Object.entries(graph)){
  ...
  assert.equal(hash(b.toString().replace(/\r\n/g,'\n')),pin.canonical,id);
