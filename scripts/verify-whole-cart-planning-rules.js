  ...
  vm.runInContext(`
    function fixture(quantities = [1,1,2]) {
      ...
      for (let i=0;i<3;i++) if (quantities[i]) {
        ...
        const payloadDigest=crypto.createHash('sha256').update(JSON.stringify(['test-only.whole-cart-request',operationId,bookingNumber,roomCode,quantity,checkIn,checkOut])).digest('hex');
        groupRequests.push({operationId,bookingNumber,roomCode,quantity,checkIn,checkOut,payloadDigest});
      }
    ...
    function requestFor(roomCode, quantity, operationId, checkIn='2028-06-10', checkOut='2028-06-12') {
      ...
      const payloadDigest=crypto.createHash('sha256').update(JSON.stringify(['test-only.whole-cart-request',operationId,bookingNumber,roomCode,quantity,checkIn,checkOut])).digest('hex');
      return {operationId,bookingNumber,payloadDigest,checkIn,checkOut,roomCode,quantity};
    }
