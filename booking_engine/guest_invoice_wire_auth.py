"""Default-OFF guest wire authentication, distinct from the legacy owner API.

Environment configuration contains channel keys only, never Wix scope-seal keys.
...
# Explicit opt-in only. Render receives channel keys, NEVER Wix scope-seal keys.
_enabled = lambda: os.environ.get('WBE_GUEST_INVOICE_ENABLED') == 'true'
_now = lambda: time.time_ns() // 1000000


def _read_configuration():
    _need(_enabled() is True)
    text = os.environ.get('WBE_GUEST_INVOICE_CHANNEL_KEYS', '')
    _need(type(text) is str and 0 < len(text) <= 16384 and text.isascii())
    return dict(siteOrigin=os.environ.get('WBE_GUEST_INVOICE_SITE_ORIGIN', ''),
                audience=os.environ.get('WBE_GUEST_INVOICE_AUDIENCE', ''),
                channel=_json(text.encode('ascii'), True))
