/** Pure configurable receipt requirement only. NOT_REQUIRED is not consent,
 ...
 * adapters must establish those facts and complete rule reads separately.
 * Complete valid rules with no true entry do not require a location observation.
...
export function resolveGuestConsentRequirement(input) {
  ...
  try {
    ...
    const locationStatus = descriptor(envelope.location, 'status');
    const unknown = locationStatus && locationStatus.value === 'UNKNOWN';
    const location = snapshot(envelope.location, unknown ? ['status'] : ['status', 'countryCode', 'usStateCode']);
    const requirements = snapshot(envelope.requirements, ['status', 'rows']);
    if (requirements.status !== 'COMPLETE') return 'UNRESOLVED';
    if (!unknown && (location.status !== 'KNOWN' || !token(location.countryCode) ||
    ...
    let required = false, anyRequired = false;
    for (let i = 0; i < rows.length; i++) {
      ...
      if (row.consentRequired === true) anyRequired = true;
      if (!unknown && row.countryCode === location.countryCode &&
    ...
    // No positive rule anywhere means geography cannot affect this decision.
    // A future positive rule restores the IP-location prerequisite, never a grant.
    if (unknown) return anyRequired ? 'UNRESOLVED' : 'NOT_REQUIRED';
