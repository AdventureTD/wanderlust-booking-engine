...
function evalSource(file,globals,names){if(read(file).includes("from 'backend/googleAdsHttpDiagnostics'")&&!Object.hasOwn(globals,'sanitizeHttpErrorDiagnostics'))globals={...globals,sanitizeHttpErrorDiagnostics:evalSource('backend/googleAdsHttpDiagnostics.js',{},['sanitizeHttpErrorDiagnostics']).sanitizeHttpErrorDiagnostics};const c=vm.createContext({Date,Buffer,console:{log(){},error(){},warn(){}},Permissions:{Anyone:'Anyone',Admin:'Admin'},webMethod:(_,f)=>f,fetch(){throw Error('NETWORK DENIED');},...globals});vm.runInContext(read(file).replace(/^import .*;\r?\n/gm,'').replace(/export /g,'')+'\nObject.assign(this,{'+names.join(',')+'});',c);return c;}
function fixture(opts={}){
 ...
 globals.sanitizeHttpErrorDiagnostics=evalSource('backend/googleAdsHttpDiagnostics.js',globals,['sanitizeHttpErrorDiagnostics']).sanitizeHttpErrorDiagnostics;
